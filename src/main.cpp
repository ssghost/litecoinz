...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, fExpensiveChecks ? verifier : disabledVerifier, chainparams.GetConsensus(), !fJustCheck, !fJustCheck))
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp)
{
    ...
    if ((!CheckBlock(block, state, verifier, chainparams.GetConsensus(), GetAdjustedTime())) || !ContextualCheckBlock(block, state, pindex->pprev)) {
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, const CNode* pfrom, const CBlock* pblock, bool fForceProcessing, const CDiskBlockPos* dbp, bool fMayBanPeerIfInvalid)
{
    {
        LOCK(cs_main);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlock(block, state, verifier, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot))
        return error("%s: Consensus::CheckBlock: %s", __func__, FormatStateMessage(state));
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(block, state, verifier, chainparams.GetConsensus()))
   ...
   if (!(pfrom->nServices & NODE_BLOOM) &&
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it->second->nHeight < chainActive.Height() - 15) {
            LogPrint(BCLog::NET, "Peer %d sent us a getblocktxn for a block > 15 deep", pfrom->id);
            return true;
        }
